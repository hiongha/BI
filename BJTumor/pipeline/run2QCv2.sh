...
cs_qc=$(ls -d $FC/*$fc_short*.xls|xargs -l basename|grep -v 'bak')
...
" > run_draw_format.sh 
echo "/share/work2/lisuxing/suxx/software/Python-3.6.1/bin/python3 $pipeline/IntegrateQC.py --bz-qc result.txt --cs-qc $cs_qc --brief-file $brief_file --type $primer_prefix" >> run_draw_format.sh 
echo "python $pipeline/mutations_csv.py --brief-file $brief_file --must-given $must_given_xls --final-report $cs_qc --type $primer_prefix_org --out-put $sample"".mutations.csv >mut.log" >> run_draw_format.sh
...
perl $pipeline/get_location.pl $pipeline/config/egfr."$primer_prefix_org"_location.txt $root_dir/reformed_bam_pos.txt > reformed_bam_pos.txt_getlocation.txt
...
perl $pipeline/get_location.pl $pipeline/config/egfr."$primer_prefix_org"_location.txt unique_bam_pos.txt > unique_bam_pos.txt_getlocation.txt
...
report_dir=$(sed -r 's/(.*_.*_.*_.*)_(.*)/\1_cSMART_'"$primer_prefix"'/' <<< $FC)
...
mv $root_dir/*.png $sample"_report"/$report_dir
if [[ $primer_prefix == "LC" ]];then
	mv $root_dir/$sample".fusion.csv" $root_dir/$sample".SnvIndel.csv" $root_dir/$sample".QC.csv" $sample"_report"/$report_dir
else
	mv $root_dir/$sample".SnvIndel.csv" $root_dir/$sample".QC.csv" $sample"_report"/$report_dir
fi
